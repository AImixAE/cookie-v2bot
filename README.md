...
## 项目描述
一个功能丰富的Telegram群组机器人，具有成就系统、徽章系统、卡片系统、等级系统和完整的管理界面，旨在增加群组活跃度和用户互动。

## 功能特性

### 核心功能
- **成就系统**：用户可以通过在群组中活动解锁各种成就
- **徽章系统**：用户可以获得和展示不同的徽章，支持重复获得并显示数量
- **卡片系统**：用户可以使用经验值购买卡片，卡片具有不同的功能和价值
- **经验值系统**：用户通过在群组中发送消息获得经验值
- **等级系统**：基于经验值的等级晋升系统，每个等级需要不同的经验值
- **排行榜系统**：支持按经验值或消息数排序的排行榜

### 管理工具
- **命令行界面 (CLI)**：通过命令行管理机器人和查看数据
- **图形用户界面 (GUI)**：使用PySide6实现的桌面应用，提供直观的数据管理和可视化
- **日志系统**：支持环境变量配置，自动清理过期日志
...
- **Python 3.12++**
- **python-telegram-bot[job-queue]**：用于实现Telegram机器人
- **SQLite**：用于本地数据库存储
- **PySide6**：用于实现图形用户界面（可选依赖）
...
3. **配置环境变量**
   ...
   - 在 `.env` 文件中填入你的 Telegram Bot Token 和其他配置
   ```env
   # Telegram Bot Token
...
4. **配置成就、徽章、卡片和等级**
   ...
   - 编辑 `config/level.toml` 配置等级
   - 编辑 `config/config.toml` 配置基本设置
...
# 查看排行榜（支持 --sort 选项：exp 或 msg）
uv run run.py cli leaderboard <chat_id> --sort exp
...
### config.toml
定义机器人的基本配置，如消息处理设置、经验值计算规则等。

### level.toml
定义等级系统配置，使用 delta 格式，列出每个等级所需的额外经验值。
...
等级系统使用 delta 格式配置，每个等级所需的额外经验值如下：
...
- `/leaderboard daily msg` - 以消息数排序查看昨日排行榜
